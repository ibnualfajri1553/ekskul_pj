...
<div class="container-fluid">
    <h2 class="fw-bold text-primary mb-4">Tambah Anggaran</h2>
    ...
    <form action="{{ route('admin.anggaran.store') }}" method="POST" class="bg-white shadow-sm p-4 rounded">
        @csrf
        ...
        <div class="mb-3">
            ...
            <input type="number" name="anggaran_total" class="form-control" value="{{ old('anggaran_total') }}"
                required>
        </div>
        ...
        <select name="tahun_anggaran" class="form-select" required>
            ...
            <option value="{{ $year }}"
                {{ old('tahun_anggaran', $anggaran->tahun_anggaran ?? '') == $year ? 'selected' : '' }}>
                {{ $year }}
            </option>
            @endfor
        </select>
